...
async def get_profiles(role_title: str, keywords: List[str], location_filter: Optional[str] = None, limit: int = 20) -> List[str]:
    ...
    user_token = settings.twitter_user_access_token  # For /2/users/search OAuth2 user
    if not bearer_token and not user_token:
        print("No Bearer or user token – Empty (real only)")
        return []

    try:
        print("Direct Bearer v2 /2/tweets/search/recent or user token /2/users/search")
        headers = {"Authorization": f"Bearer {bearer_token or user_token}"}
        async with httpx.AsyncClient(timeout=30.0) as client:
            # Primary: Tweet search (public, Bearer)
            query_terms = [role_title] + keywords
            query = " ".join(query_terms) + " -is:retweet lang:en"
            if location_filter:
                query += f" {location_filter}"  # Approx keyword
            ...
            if tweets_resp.status_code != 200:
                print(f"Tweets error: {tweets_resp.text}")
                # Fallback user search if user token (OAuth2 user context)
                if user_token and 'user/search' in str(tweets_resp.text):  # Or status 403 unsupported
                    print("Fallback to /2/users/search (user token)")
                    query_enc = urllib.parse.quote(f"{role_title} {' '.join(keywords)}")
                    users_url = f"https://api.twitter.com/2/users/search?query={query_enc}&max_results={min(limit, 50)}"
                    users_resp = await client.get(users_url, headers=headers)
                    print(f"Users search status: {users_resp.status_code}")
                    if users_resp.status_code == 200:
                        users = users_resp.json().get("data", [])
                    else:
                        print(f"Users error: {users_resp.text}")
                        return []
                else:
                    return []  # Real only
            else:
                    ...
                    for user in users:
                        ...
                        if pinned_id:
                            ...
                            if pinned_resp.status_code == 200:
                                ...
                                profile_text += f". Pinned: {pinned.get('text', '')[:280]} (likes {pinned.get('public_metrics', {}).get('like_count', 0)})"

                        tweets_url = f"https://api.twitter.com/2/users/{user['id']}/tweets?max_results=10&tweet.fields=text,public_metrics&exclude=replies"
                        tweets_resp = await client.get(tweets_url, headers=headers)
                        if tweets_resp.status_code == 200:
                            ...
                            profile_text += f". Tweets (likes {likes_total}): {tweet_summary}..."

                        profiles.append(profile_text)

                print(f"v2 workflow: {len(tweets)} tweets → {len(profiles)} enriched profiles for Grok")

    except Exception as e:
        print(f"X API workflow error: {e}")
        return []  # Real only
